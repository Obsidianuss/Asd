...
void CHARACTER::Initialize()
{
	...
	memset(&m_bSkillTreeLevel, 0, sizeof(m_bSkillTreeLevel));
	m_iSkillTreeTime = 0;
#endif

#ifdef __SKILL_TREE__
void CHARACTER::SendSkillTreeData()
{
	SkillTreeFile(false);
	BYTE j = 0;
	for (j = 0; j < 33; ++j)
	{
		if (GetSkillTreeLevel(j) > 0)
			break;
		if (j == 32)
			return;
	}
	...
	for (j = 0; j < 33; ++j)
	{
		if (GetSkillTreeLevel(j) > 0)
		{
			commandStr += std::to_string(j);
			commandStr += "?";
			commandStr += std::to_string(GetSkillTreeLevel(j));
			commandStr += "#";
		}
	}
	const CAffect* affect = FindAffect(AFFECT_SKILL_TREE);
	ChatPacket(CHAT_TYPE_COMMAND, "SkillTreeLoad %s %d %d", commandStr.c_str(), affect ? affect->lDuration : m_iSkillTreeTime, affect ? 1 : 0);
}
void CHARACTER::SkillTreeSetStatus(bool bStatus)
{
	int sendTime = m_iSkillTreeTime;

	if (!bStatus)
	{
		const CAffect* affect = FindAffect(AFFECT_SKILL_TREE);
		m_iSkillTreeTime = affect ? affect->lDuration : 0;
		sendTime = m_iSkillTreeTime;
		RemoveAffect(AFFECT_SKILL_TREE);
	}
	else
	{
		RemoveAffect(AFFECT_SKILL_TREE);
		if (m_iSkillTreeTime > 0)
		{
			BYTE j = 0;
			for (j = 0; j < 33; ++j)
			{
			...
			if (j != 99)
			{
				for (j = 0; j < 33; ++j)
				{
					if (GetSkillTreeLevel(j) > 0)
					{
						itertype(m_mapSkillTree) skillData = m_mapSkillTree.find(j);
						if (skillData == m_mapSkillTree.end())
							continue;
						long extraValue = 0;
						CAffect* oldAffect = FindAffect(AFFECT_SKILL_TREE, skillData->second.pointIdx);
						if(oldAffect)
						{
							extraValue = oldAffect->lApplyValue;
							RemoveAffect(oldAffect);
						
						}
						AddAffect(AFFECT_SKILL_TREE, skillData->second.pointIdx, skillData->second.pointValues[GetSkillTreeLevel(j) - 1] + extraValue, AFF_NONE, m_iSkillTreeTime, 0, false);
					}
				}
				m_iSkillTreeTime = 0;
			}
...
bool CHARACTER::SkillTreeRefill()
{
	constexpr DWORD needItems[][2] = {
		{51501, 1000},
		{51502, 1000},
		{51503, 1000}
	};

	if (FindAffect(AFFECT_SKILL_TREE))
		return false;
	else if (m_iSkillTreeTime != 0)
		return false;
	for (BYTE j = 0; j < 33; ++j)
	{
		if (GetSkillTreeLevel(j) > 0)
	...
	for (BYTE j = 0; j < _countof(needItems); ++j)
	{
		if (needItems[j][0] == 1)
		{
			if (GetGold() < needItems[j][1])
			{
				ChatPacket(CHAT_TYPE_INFO, "You not has enought money!");
				return false;
			}
		}
		else
		{
			if (CountSpecifyItem(needItems[j][0]) < needItems[j][1])
			{
				const TItemTable* itemTable = ITEM_MANAGER::instance().GetTable(needItems[j][0]);
				ChatPacket(CHAT_TYPE_INFO, "You not has enought item! x%u %s", needItems[j][1], itemTable ? itemTable->szLocaleName : "None");
				return false;
			}
		}
	}
	...
	{
		if (needItems[j][0] == 1)
			PointChange(POINT_GOLD, -(long)needItems[j][1]);
		else
			RemoveSpecifyItem(needItems[j][0], needItems[j][1]);
	}

	m_iSkillTreeTime = 60 * 60 * 24;
	ChatPacket(CHAT_TYPE_COMMAND, "SkillTreeSetTime %d", m_iSkillTreeTime);
	SkillTreeFile(true);
	return true;
}
BYTE CHARACTER::GetSkillTreeMaxLevel(BYTE skillIdx)
{
	itertype(m_mapSkillTree) skillData = m_mapSkillTree.find(skillIdx);
	if (skillData == m_mapSkillTree.end())
		return false;
	const TSkillTree& skill = skillData->second;
	BYTE maxRefine = 0;
	for (BYTE j = 0; j < 10; ++j)
	{
		if (skill.refineList[j] >= 0)
			maxRefine += 1;
	}
	return maxRefine;
}
bool CHARACTER::SkillTreeRefine(BYTE skillIdx, bool refineType)
{
	if (GetLevel() < 90)
		return false;
	if (refineType && CountSpecifyItem(72347) < 1)
	{
		ChatPacket(CHAT_TYPE_INFO, "You don't has skill tree booster!");
		return false;
	}
	const itertype(m_mapSkillTree) skillData = m_mapSkillTree.find(skillIdx);
	if (skillData == m_mapSkillTree.end())
		return false;
	const TSkillTree& skill = skillData->second;
	if (FindAffect(AFFECT_SKILL_TREE))
		return false;
	if (GetSkillTreeLevel(skillIdx) >= GetSkillTreeMaxLevel(skillIdx))
		return false;
	if (skill.skillConnect != -1 && GetSkillTreeLevel(skill.skillConnect) != GetSkillTreeMaxLevel(skill.skillConnect))
		return false;

	const int refineIdx = skill.refineList[GetSkillTreeLevel(skillIdx)];

	const itertype(skillTreeRefineList) refineItems = skillTreeRefineList.find(refineIdx);
	if (refineItems == skillTreeRefineList.end())
		return false;

	const std::vector<std::pair<DWORD, long>>& refineList = refineItems->second;

	for (BYTE j = 0; j < refineList.size(); ++j)
	{
		if (refineList[j].first == 1)
		{
			if (GetGold() < refineList[j].second)
			{
				ChatPacket(CHAT_TYPE_INFO, "You not has enought money!");
				return false;
			}
		...
		{
			if (CountSpecifyItem(refineList[j].first) < (!refineType ? refineList[j].second : int((float(refineList[j].second)/2.0) + 0.5)))
			{
				const TItemTable* itemTable = ITEM_MANAGER::instance().GetTable(refineList[j].first);
				ChatPacket(CHAT_TYPE_INFO, "You not has enought item! x%d %s", (!refineType ? refineList[j].second : refineList[j].second / 2), itemTable ? itemTable->szLocaleName : "None");
				return false;
			}
		}
	}
	...
	{
		if (refineList[j].first == 1)
			PointChange(POINT_GOLD, -refineList[j].second);
		else
			RemoveSpecifyItem(refineList[j].first, refineType ? int((float(refineList[j].second) / 2.0) + 0.5) : refineList[j].second);
	}

	if (refineType)
		RemoveSpecifyItem(72347);

	m_bSkillTreeLevel[skillIdx] += 1;
	ChatPacket(CHAT_TYPE_COMMAND, "SkillTreeSetLevel %u %u", skillIdx, m_bSkillTreeLevel[skillIdx]);
	SkillTreeFile(true);
	return true;
}
void CHARACTER::SkillTreeFile(bool bSave)
{
	char filename[124];
	snprintf(filename, sizeof(filename), "share/skill_tree/%u",GetPlayerID());
	FILE* fp;
	if (bSave)
	{
		if ((fp = fopen(filename, "w+")) != NULL)
		{
			...
			for (BYTE j = 0; j < 33; ++j)
			{
				if(m_bSkillTreeLevel[j] > 0)
					fprintf(fp, "%u %u\n", j, m_bSkillTreeLevel[j]);
			}
			fclose(fp);
		}
	}
	else
	{
		...
		if ((fp = fopen(filename, "r")) != NULL)
		{
			...
			while (fgets(one_line, 256, fp))
			{
				...
				if (vecArgs[0] == "time")
					str_to_number(m_iSkillTreeTime, vecArgs[1].c_str());
				else
				{
					WORD skillIndex, skillLevel;
					str_to_number(skillIndex, vecArgs[0].c_str());
					str_to_number(skillLevel, vecArgs[1].c_str());
					if (skillIndex > 32)
						continue;
					m_bSkillTreeLevel[skillIndex] = skillLevel;
				}
			}
			fclose(fp);
		}
			...
			if (amount > 0)
			{
				...
				switch (val)
				{
					...
					case 3:
						...
						if (GetLevel() < 250) PointChange(POINT_STAT, 1); 
						break;
...
void CHARACTER::OpenMyOfflineShop(const char * c_pszSign, TOfflineShopItemTable * pTable, BYTE bItemCount, BYTE bTime)
{
...

	{
		if (!test_server)
		{
			if (IsGM())
			{
				ChatPacket(CHAT_TYPE_INFO, "You cannot open offline-shop! You're a gamemaster");
				return;
			}
		}
	}

	{
		...
		if (bResult)
		{
			ChatPacket(CHAT_TYPE_INFO, "At first, you have to get your unsold items at admin panel.");
			return;
		}
	}

	{
		...
		if (bResult)
		{
			ChatPacket(CHAT_TYPE_INFO, "You already have an offline-shop.");
			ChatPacket(CHAT_TYPE_INFO, "You cannot open a new offline-shop before close old one.");
			return;
		}
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "You can't use this offline shop title!");
		return;
	}
	...
	for (BYTE i = 0; i < bItemCount; ++i)
	{
		...
		{
			bIsWrong = true;
			ChatPacket(CHAT_TYPE_INFO, "You can't put the equipped items.");
			return;
		}
		...
		{
			bIsWrong = true;
			ChatPacket(CHAT_TYPE_INFO, "You can't sell the locked items!");
			return;
		}
	...
	snprintf(szName, sizeof(szName), "%s's shop", GetName());
	npc->SetName(szName);
	// End Of Set Name
	...
	if (bTime != 4)
		ChatPacket(CHAT_TYPE_INFO, "<Offline Shop> Your offline shop's time is expired after %d hour(s)", iTime / 60 / 60);
	else
...
EVENTFUNC(offline_shop_update_event)
{
	...
	if (remain_time <= 0)
	{
		...
		LPCHARACTER pChar = CHARACTER_MANAGER::Instance().FindByPID(npc->GetOfflineShopRealOwner());
		if (pChar)
		{
			pChar->ChatPacket(CHAT_TYPE_INFO, "Your offline shop timer is over. You can check your unsold items at get back window.");
...
void CHARACTER::ResetPoint(int iLv)
{
	...
	PointChange(POINT_STAT, ((MINMAX(1, iLv, 1200) - 1) * 3) + GetPoint(POINT_LEVEL_STEP) - GetPoint(POINT_STAT));
...
void CHARACTER::AddSashMaterial(TItemPos tPos, BYTE bPos)
{
	...
	else if ((bPos == 1) && (m_bSashAbsorption))
	{
		...
		else if ((pkItem->GetType() != ITEM_WEAPON) && (pkItem->GetSubType() != ARMOR_BODY))
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You can absorb just the bonuses weapons."));
			return;
		}
	}
...
bool CHARACTER::CheckUseItemAntiFlood(LPITEM pItem)
{
	...
	// If you use the Sanii switch bot please complete in this condition
	if(pItem->GetSubType() == USE_CHANGE_ATTRIBUTE || pItem->GetSubType() == USE_CHANGE_ATTRIBUTE2)
		return false;

	const int iUseItemLimitPerSec = 10; // 10 clicks per 1 second
