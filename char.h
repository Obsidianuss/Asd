...
public:
	void			SkillTreeFile(bool bSave);
	bool			SkillTreeRefine(BYTE skillIdx, bool refineType);
	bool			SkillTreeRefill();
	void			SkillTreeSetStatus(bool bStatus);
	void			SendSkillTreeData();

	BYTE			GetSkillTreeLevel(BYTE bIdx) { return m_bSkillTreeLevel[bIdx]; }
	BYTE			GetSkillTreeMaxLevel(BYTE skillIdx);
protected:
	BYTE			m_bSkillTreeLevel[33];
	int				m_iSkillTreeTime;
#endif
	public:
...
//		bool			IsPet() const		{ return m_bCharType == CHAR_TYPE_PET; }
	...
	public:
		...
		void			SetOfflineShopStatus(BYTE bStatus) { m_bOfflineShopStatus = bStatus; }
		BYTE			GetOfflineShopStatus() const { return m_bOfflineShopStatus; }
	...
	public:
		...
		bool				__Profile__Damage(LPCHARACTER pAttacker, long long dam, EDamageType type = DAMAGE_TYPE_NORMAL);
...
#endif
	public:
		void				SetGamblingChance(BYTE chance){SetQuestFlag("pacanele.gamble", chance);}
		void				SetGamblingMoney(long long money){SetQuestFlag("pacanele.power", money);}
		BYTE				GetGamblingChance() {return GetQuestFlag("pacanele.gamble");}
		long long 			GetGamblingMoney() {return GetQuestFlag("pacanele.power");}
		int		GetPacaneleTimer() const { return m_ipacanele; }
		void	SetPaceneleTimer(int pulse) { m_ipacanele = pulse; }

		LPEVENT				m_pkPacanele;
