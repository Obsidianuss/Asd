...
typedef struct SSkillTree
{
	BYTE pointIdx;
	WORD pointValues[10];//maxRefine!
	int refineList[10];//maxRefine!
	int skillConnect;//don't touch!
	SSkillTree(BYTE _pointIdx, const std::vector<WORD>& _pointValues, const std::vector<int>& _refineList, int _skillConnect) : pointIdx(_pointIdx), skillConnect(_skillConnect)
	{
		memset(&pointValues, 0, sizeof(pointValues));
		memset(&refineList, -1, sizeof(refineList));
		for (BYTE j = 0; j < _pointValues.size(); ++j)
		{
			if (j >= _countof(pointValues))
				break;
			pointValues[j] = _pointValues[j];
		}
		for (BYTE j = 0; j < _refineList.size(); ++j)
		{
			if (j >= _countof(refineList))
				break;
			refineList[j] = _refineList[j];
		}
	}
}TSkillTree;
extern std::map<BYTE, TSkillTree> m_mapSkillTree;
extern std::map<BYTE, std::vector<std::pair<DWORD, long>>> skillTreeRefineList;
#endif
...
// Always 100% drop chance, no matter the level difference
#define PERCENT_LVDELTA(me, victim) 100
#define PERCENT_LVDELTA_BOSS(me, victim) 100

// Calculating value with a 100% drop chance
#define CALCULATE_VALUE_LVDELTA(me, victim, val) val
